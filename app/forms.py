...
class BlogForm (forms.ModelForm):
    class Meta:
        ...
        # заполняемые поля для ввода заголовка, краткого содержания, полного содержания статьи, выбора файла картинки и ссылки на видео
        fields = ('title', 'description', 'content', 'image', 'video_url',)
        labels = {
            'title': "Заголовок",
            'description': "Краткое содержание",
            'content': "Полное содержание",
            'image': "Картинка",
            'video_url': "Видео (ссылка на ролик, например YouTube)"
        }  # метки к полям формы


# Код для формы записи на приём к врачу:
class AppointmentForm(forms.ModelForm):
    class Meta:
        model = Appointment # Используемая модель
        fields = ('patient_name', 'doctor', 'appointment_date', 'appointment_time', 'phone', 'email', 'comment',)
        labels = {
        ...
        widgets = {
            'patient_name': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Введите ваше ФИО'}),
            ...
            'comment': forms.Textarea(attrs={'class': 'form-control', 'rows': 4, 'placeholder': 'Дополнительная информация'}),
        }
    
    def __init__(self, *args, **kwargs):
        super(AppointmentForm, self).__init__(*args, **kwargs)
        # Фильтруем только активных врачей
        self.fields['doctor'].queryset = Doctor.objects.filter(is_active=True).order_by('specialty', 'full_name')
