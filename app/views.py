...
from .models import Feedback # использование модели отзывов
from .forms import CommentForm # использование формы ввода комментария

from .forms import BlogForm # использование формы для ввода статьи блога
from .forms import AppointmentForm # использование формы записи на приём
from .models import Appointment # использование модели записи на приём
    ...
    if request.method == "POST": # после отправки данных формы на сервер методом POST
        ...
        # логическое значение, информирующее об успешной проверке. 
            ...
            feedback = Feedback.objects.create(
                ...
                notice=form.cleaned_data.get('notice', False),
                contact=form.cleaned_data.get('contact', False),
...
def contact(request):
    ...
    if request.method == "POST": # после отправки формы записи на сервер методом POST
        appointment_form = AppointmentForm(request.POST)
        if appointment_form.is_valid(): # выполняется валидация полей формы
            # Проверяем, не занято ли это время у выбранного врача
            ...
            # Проверяем, не существует ли уже запись на это время
            ...
            if existing_appointment:
                # Если время занято, добавляем ошибку
                appointment_form.add_error('appointment_time', 'Это время уже занято. Пожалуйста, выберите другое время.')
            else:
                # Сохраняем запись
                appointment = appointment_form.save()
                appointment_success = True
                appointment_form = None # скрываем форму после успешной отправки
    else:
        appointment_form = AppointmentForm() # создание пустой формы для записи на приём
    ...
    return render(
        ...
        'app/contact.html',
        {
            ...
            'year':datetime.now().year,
            'appointment_form': appointment_form, # передача формы записи в шаблон
            'appointment_success': appointment_success, # флаг успешной записи
        }
    )
...
def appointments(request):
    """Renders the appointments page - список всех записей на приём."""
    assert isinstance(request, HttpRequest)
    # Получаем все записи, отсортированные по дате (сначала ближайшие)
    ...
    return render(
        ...
        'app/appointments.html',
        {
            ...
            'message':'Список всех записей на приём к врачам',
